object Exercises {
  ...
  def primeFactor(number: Int): Seq[Int] = {
    if (number == 1) Seq(1)
    else if (simple(number)) Seq(number)
    else {
      for {i <- 2 until number
           if simple(i) && number % i == 0} yield i
    }
  }
  ...
  */
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
    ballsArray.toSeq.sortBy(x => x._2._1 * x._2._2 * x._2._2 * x._2._2).map(x => x._1)
}